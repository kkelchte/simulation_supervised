...
PARAMS="${@:5}"
WORLDS=(sandbox sandbox canyon forest)
COUNTSUC=(0 0 0 0) 
COUNTTOT=(0 0 0 0)
...
# debug space
# xterm -l -lf $LLOC/xterm_log/df_log -hold -e "while [ true ] ; do echo \$(date +%F_%H%M%S); df -h /tmp ; df -h / ; df -h /esat/qayd/kkelchte; df -h .; echo; echo; sleep 60; done" &

# for i in $(seq 0 $((NUMBER_OF_FLIGHTS-1))) ;
i=$START_INDEX
while [[ $i -lt $((NUMBER_OF_FLIGHTS+START_INDEX)) ]] ;
do
  NUM=$((i%${#WORLDS[@]}))
  ...
  if [ ${WORLDS[NUM]} == sandbox ] ; then
    SANDLOC=$(rospack find simulation_supervised_demo)/worlds/sandboxes_val
  ...
  COMMANDR="roslaunch simulation_supervised_demo $LAUNCHFILE\
   ...
   docker:=$docker recovery:=$RECOVERY $EXTRA_ARGUMENTS"
  echo $COMMANDR
  START=$(date +%s)     
