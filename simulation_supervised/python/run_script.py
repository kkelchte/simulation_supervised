...
parser.add_argument("--z_pos",default=1,type=float, help="Specify z position.")
...
while run_number < FLAGS.number_of_runs:
  ...
  evaluate=((run_number%FLAGS.evaluate_every) == 1 and run_number != 1) or FLAGS.evaluation
  ...
  if world_name in ['canyon', 'forest', 'sandbox'] and not FLAGS.reuse_default_world:
    ...
    command="{0} world_file:={1}".format(command, world_file)
    world_image='{0}/../simulation_supervised_demo/worlds/{2}_evaluation/{1:05d}_{2}.png'.format(simulation_supervised_dir,run_number%10, world_name)
