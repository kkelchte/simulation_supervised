...
def shutdown():
  ...
  if stop_nn_pub: stop_nn_pub.publish(Empty()) # let NN know that there is a break.
...
def depth_cb(msg):
  ...
  try:
    ...
    distance = np.nanmin(de)
  except CvBridgeError, e:
    print(e)
  else:
    if distance < min_distance and not shuttingdown and ready:
      print('[fsm.py]: {0}: bump @ {1}'.format(rospy.get_time(), time.time()))
      success=False
      shutdown()
...
def scan_cb(data):
  ...
  if min(ranges) < min_distance and not shuttingdown:
    print('[fsm.py]: {0}: bump @ {1}'.format(rospy.get_time(), time.time()))
    success=False
    shutdown()
...
def gt_cb(data):
  ...
  global current_pos, ready, success, positions, start_time
  current_pos=[data.pose.pose.position.x,
              data.pose.pose.position.y,
              data.pose.pose.position.z]
  positions.append(current_pos)
  if (starting_height==-1 or current_pos[2] >= starting_height) and not shuttingdown:
    print('[fsm.py]: {0} = {1} - {2}: ready!'.format(rospy.get_time()-start_time, rospy.get_time(), start_time))
...
if __name__=="__main__":
	...
	if rosparam.has_param('state_sequence'):
		state_sequence=rosparam.get_param('state_sequence')
	if rosparam.has_param('control_sequence'):
		control_sequence=rosparam.get_param('control_sequence')
	if rosparam.has_param('supervision_sequence'):
		supervision_sequence=rosparam.get_param('supervision_sequence')
	...
	if 'CON' in control_sequence.values():
		rospy.Subscribe('/overtake', Empty, overtake_cb)
	...
	# DH: switch between running and not running
	...
	if len(state_sequence) > 1: 
		rospy.Subscribe('/go', Empty, go_cb)
  ...
  if rospy.has_param('min_distance'): 
